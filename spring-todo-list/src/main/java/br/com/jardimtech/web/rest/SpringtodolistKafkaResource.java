...
@RestController
@RequestMapping("/api/springtodolist-kafka")
public class SpringtodolistKafkaResource {

    private final Logger log = LoggerFactory.getLogger(SpringtodolistKafkaResource.class);

    private final MessageChannel output;
    private Sinks.Many<Message<String>> sink = Sinks.many().unicast().onBackpressureBuffer();

    public SpringtodolistKafkaResource(@Qualifier(KafkaSseProducer.CHANNELNAME) MessageChannel output) {
        this.output = output;
    }
